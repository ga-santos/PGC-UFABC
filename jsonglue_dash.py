...
def printResults(full):
    ...
    for filecomp in full:
        ...
        for comparison in filecomp[2]:
            ...
            if Sin is None or Sem is None or Hist is None: continue
            if float(Sin) <= 0.2 or float(Sem) <= 0.2 or float(Hist) <= 3.5:
                printAux(comparison)
                print("             ", end="")
...
def leituraSchemas(argumentos):
    ...
    arguments = [' '] + argumentos.split(" ") #ADICIONA UMA POSICAO VAZIA POIS NO ORIGINAL IA O CAMINHO DO ARQUIVO
    flags = []
...
def compararSchemas(x, y, part):
    ...
    comp = cp.compGraph(graphs_dict[filename[x]], graphs_dict[filename[y]], graphs_size[filename[x]],
                        graphs_size[filename[y]], x, y, part)
